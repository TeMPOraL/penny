...
(defmethod to-spoken-string ((memo memo))
  (format nil "Memo from ~A sent ~A at ~A ⇒ ~A" (author memo) (format-date (send-time memo)) (format-time (send-time memo)) (text memo)))
...
(register-matcher :delayed-notify-user
                  (list (match-score (lambda (input)
                                       (and (directedp input)
                                            (or (mentions "przypomnij" (unquoted-part input)) ;NOTE left for alternative translation
...
(provide-output :more-memos '("There are more memos!"
                              "I have more messages for you!"
                              "But that's not all!"
                              "There is more! :)"
                              "I have something more to tell you."
                              "There's more where this came from."))

(provide-output :memo-saved '("saved memo"
                              "saved as a memo"
                              "saved; I'll tell when I see them"
                              "sure, saved"
                              "ok, I'll pass it on"))

(provide-output :memo-failed  '("Can't send this memo. Not sure I know who you're talking about."
                                "Not sure what you want sent to whom."
                                "Recipient unknown. *sigh*"
                                "I'm not really sure who to deliver it to..."))

(provide-output :notification-sent '("ok, delivered"
                                     "sure, sent"
                                     "sure, sending it now"
                                     "message sent!"
                                     "sent!"))

(provide-output :failed-in-sending-notification '("Something broke :(."
                                                  "Something's not working. *sigh*"
                                                  "I cannot into notifications. *sob*"))

(provide-output :failed-to-extract-timestring '("I can't understand when..."
                                                "Please specify more accurately when."
                                                "Again - when?"))

(provide-output :delayed-memo-saved '("Ok."
                                      "Copy that."
                                      "Saved for delivering later."))
